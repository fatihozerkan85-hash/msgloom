      ...
      <div className="grid lg:grid-cols-2 gap-8">
        <form onSubmit={handleSend} className="bg-white rounded-2xl border border-gray-100 p-8 space-y-5">
          <div>
            ...
            <input type="text" value={phone} onChange={(e) => setPhone(e.target.value)}
              ...
              className="w-full border border-gray-200 rounded-xl px-4 py-3 focus:ring-2 focus:ring-green-500 focus:border-transparent focus:outline-none transition" />
          ...
          <div>
            ...
            <div className="flex gap-3">
              <button type="button" onClick={() => setType('text')}
                className={`flex-1 py-3 rounded-xl text-sm font-medium border-2 transition ${type === 'text' ? 'border-green-500 bg-green-50 text-green-700' : 'border-gray-200 text-gray-600 hover:border-gray-300'}`}>
                ✏️ Metin Mesajı
              </button>
              <button type="button" onClick={() => setType('template')}
                className={`flex-1 py-3 rounded-xl text-sm font-medium border-2 transition ${type === 'template' ? 'border-green-500 bg-green-50 text-green-700' : 'border-gray-200 text-gray-600 hover:border-gray-300'}`}>
                📋 Template
              </button>
            </div>
          </div>
          ...
          {type === 'text' ? (
            <div>
              ...
              <textarea value={message} onChange={(e) => setMessage(e.target.value)} rows={5}
                ...
                className="w-full border border-gray-200 rounded-xl px-4 py-3 focus:ring-2 focus:ring-green-500 focus:border-transparent focus:outline-none transition resize-none" />
            </div>
          ) : (
            <div>
          ...
          <button type="submit" disabled={sending}
            className="w-full bg-green-600 text-white py-3.5 rounded-xl font-medium hover:bg-green-700 disabled:opacity-50 transition shadow-sm flex items-center justify-center gap-2">
            {sending ? <><div className="animate-spin rounded-full h-4 w-4 border-b-2 border-white"></div> Gönderiliyor...</> : '📤 Mesaj Gönder'}
          </button>

          {status && (
            <div className={`p-4 rounded-xl text-sm ${status.type === 'success' ? 'bg-green-50 text-green-700 border border-green-100' : 'bg-red-50 text-red-700 border border-red-100'}`}>
              {status.text}
            </div>
          )}
        </form>
        ...
        <div className="space-y-4">
          <div className="bg-amber-50 rounded-2xl border border-amber-100 p-6">
            <h3 className="font-bold text-amber-800 text-sm mb-2">⚠️ Önemli Bilgi</h3>
          ...
          <div className="bg-blue-50 rounded-2xl border border-blue-100 p-6">
            <h3 className="font-bold text-blue-800 text-sm mb-2">💡 İpucu</h3>
