...
const stepIcons = [LinkIcon, Settings, Zap, LineChart];
...
const stepImages = [
  'https://images.unsplash.com/photo-1579563231082-c391d12ed2d4?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&q=80&w=1080',
  'https://images.unsplash.com/photo-1575388902449-6bca946ad549?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&q=80&w=1080',
  'https://images.unsplash.com/photo-1769839271832-cfd7a1f6854f?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&q=80&w=1080',
  'https://images.unsplash.com/photo-1748439281934-2803c6a3ee36?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&q=80&w=1080',
];
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <motion.div className="text-center mb-20" initial={{ opacity: 0, y: -30 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }} viewport={{ once: true }}>
          <motion.div className="inline-flex items-center gap-2 bg-white/80 backdrop-blur-sm border border-blue-200 px-6 py-3 rounded-full mb-6 shadow-lg" animate={{ y: [0, -8, 0] }} transition={{ duration: 3, repeat: Infinity, ease: 'easeInOut' }}>
            ...
            <span className="text-blue-700 font-semibold">{t(c,'howItWorks','badge')}</span>
          </motion.div>
          <h2 className="text-5xl md:text-6xl font-bold text-gray-900 mb-6 leading-tight">{t(c,'howItWorks','title')}</h2>
          <p className="text-xl text-gray-600 max-w-3xl mx-auto">{t(c,'howItWorks','subtitle')}</p>
        </motion.div>

        <div className="space-y-24">
          {stepKeys.map((sk, index) => {
            const Icon = stepIcons[index];
            const color = stepColors[index];
            const image = stepImages[index];
            const number = t(c,'howItWorks',`${sk}_number`);
            const title = t(c,'howItWorks',`${sk}_title`);
            const desc = t(c,'howItWorks',`${sk}_desc`);
            const feats = [t(c,'howItWorks',`${sk}_feat1`), t(c,'howItWorks',`${sk}_feat2`), t(c,'howItWorks',`${sk}_feat3`)];

            return (
              <motion.div key={sk} className="grid md:grid-cols-2 gap-12 items-center" initial={{ opacity: 0, y: 50 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, delay: index * 0.1 }} viewport={{ once: true }}>
                <div className={index % 2 === 1 ? 'md:order-2' : ''}>
                  <motion.div className="relative inline-block mb-6" whileHover={{ scale: 1.05 }}>
                    <motion.div className={`text-7xl md:text-8xl font-bold bg-gradient-to-br ${color} bg-clip-text text-transparent`} animate={{ opacity: [0.5, 1, 0.5] }} transition={{ duration: 3, repeat: Infinity, ease: 'easeInOut' }}>
                      {number}
                    </motion.div>
                  </motion.div>
                  <div className="flex items-center gap-3 mb-4">
                    <motion.div className={`p-3 bg-gradient-to-br ${color} rounded-xl shadow-lg`} whileHover={{ rotate: 360, scale: 1.1 }} transition={{ duration: 0.6 }}>
                      <Icon className="w-6 h-6 text-white" />
                    </motion.div>
                    <h3 className="text-3xl md:text-4xl font-bold text-gray-900">{title}</h3>
                  </div>
                  <p className="text-xl text-gray-600 mb-8 leading-relaxed">{desc}</p>
                ...
                <motion.div className={index % 2 === 1 ? 'md:order-1' : ''} whileHover={{ scale: 1.02 }} transition={{ duration: 0.3 }}>
                  <div className="relative group">
                    <motion.div className={`absolute -inset-1 bg-gradient-to-br ${color} rounded-2xl blur opacity-25 group-hover:opacity-40 transition-opacity`} animate={{ scale: [1, 1.05, 1] }} transition={{ duration: 3, repeat: Infinity, ease: 'easeInOut' }} />
                    <div className="relative h-80 md:h-96 rounded-2xl overflow-hidden shadow-2xl">
                      ...
                      <motion.div className="absolute top-6 right-6 bg-white/90 backdrop-blur-sm rounded-full w-16 h-16 flex items-center justify-center shadow-xl" animate={{ y: [0, -10, 0] }} transition={{ duration: 2, repeat: Infinity, ease: 'easeInOut' }}>
                        <span className={`text-2xl font-bold bg-gradient-to-br ${color} bg-clip-text text-transparent`}>{number}</span>
                      </motion.div>
                    </div>
                  </div>
                </motion.div>
        ...
        <motion.div className="mt-20 text-center" initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }} viewport={{ once: true }}>
          <div className="bg-white/80 backdrop-blur-sm border border-gray-200 rounded-2xl p-10 shadow-xl max-w-3xl mx-auto">
            <h3 className="text-3xl font-bold text-gray-900 mb-4">{t(c,'howItWorks','cta_title')}</h3>
            <p className="text-xl text-gray-600 mb-8">{t(c,'howItWorks','cta_desc')}</p>
            <motion.a href="/register" className="bg-gradient-to-r from-blue-600 to-indigo-600 text-white px-10 py-4 rounded-xl font-semibold text-lg shadow-lg inline-flex items-center gap-3" whileHover={{ scale: 1.05, boxShadow: '0 25px 50px -12px rgba(59, 130, 246, 0.5)' }} whileTap={{ scale: 0.95 }}>
              {t(c,'howItWorks','cta_button')}
              <motion.div animate={{ x: [0, 5, 0] }} transition={{ duration: 1.5, repeat: Infinity }}>
                <ArrowRight className="w-5 h-5" />
              </motion.div>
            </motion.a>
            <p className="text-sm text-gray-500 mt-4">{t(c,'howItWorks','cta_sub')}</p>
          </div>
        </motion.div>
      </div>
    </section>
